...
| AMFI/dyld refuses any unsigned Mach-O on a device | **MEASURED — true for unsigned, FALSE for the conclusion drawn from it** | iPad Air 4, 2026-09-18. Unsigned is refused by dyld (*"has no cdhash… has to be at least ad-hoc signed"*); ad-hoc is refused by AMFI (EPERM); **a team-signed dylib staged into `Documents` at runtime loaded AND EXECUTED** — its constructor ran in-process. **Location is not the gate; signature trust is.** Caveat: the device is in Developer Mode with a development profile, so whether an ordinary device trusts such a signature is open (#292). |
...
| No fork/exec a store will accept | **MEASURED — the SANDBOX refuses, on every channel** | `posix_spawn(self)` → EPERM with no child created (the child's own marker file absent); `fork()` → −1 EPERM. iPad Air 4, signed device build, 2026-09-18. Not a store rule — there is **no fork/exec at all** — so the right conclusion was reached for the wrong reason, and out-of-process isolation is closed on iOS permanently. **One of the few limits this audit confirmed rather than overturned.** |
...
| Android's marginal cost per extra page is ~8 MB | **CORRECTED — it is ~37 MB** | The ~8 MB figure was measured with much lighter pages. Re-measured 2026-09-22 with the *same* page weight as the iOS run (24 MB heap, 3000 DOM nodes, rAF canvas): **~36.6 MB/page** in one app process. Like-for-like: **37 MB Android shared / 114 MB Android per-process / 93 MB iOS**. An error in this ledger, corrected by re-measuring rather than by argument. |
| Android cannot give per-module webview isolation | **MEASURED FALSE in the per-app-process shape** | Three activities in `:modN` with per-process `setDataDirectorySuffix` gave 3 app processes, 3 renderer pids, 3 isolated uids. `Page.crash` on one removed only its renderer; `onRenderProcessGone` fired only in the owning process; siblings kept advancing; modules **outlived the core process**. **But only the foreground module executes** — the ROM freezes backgrounded module processes (`cgroup.freeze=1`), and even with a foreground service Chromium throttles a hidden page to ~1 Hz then stops it. Isolation is real; concurrency is not. Storage follows the **data-directory suffix**, not the process, and a live suffix collision is a fatal `RuntimeException`. |
